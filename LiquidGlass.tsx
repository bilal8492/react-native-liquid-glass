...
import { Canvas, BackdropFilter, ImageFilter, Skia, TileMode, BlendMode, SkSize, vec, mix, processTransform2d, processUniforms, SkShader, ColorChannel } from '@shopify/react-native-skia';
...
half4 main(float2 p) {
  ...
  float centerFactor = clamp(-d / r, 0.0, 1.0);
    ...
    const filter = useDerivedValue(() => {
        ...
        const filter2 = (baseShader: SkShader) => {
            ...
            const whiteTint = Skia.ImageFilter.MakeShader(
                Skia.Shader.MakeColor(Skia.Color("rgba(255, 255, 255, 0.2)"))
            );
